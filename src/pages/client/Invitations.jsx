...
const columns = [
...

  {
    ...
    format: (value, row) => (
      ...
      <MuiTypography variant="body2">
        ...
        {row.eventId?.name || row.event?.name || '—'}
...

  {
    ...
    format: (value, row) => (
      ...
      <MuiChip
        ...
        label={value === 'sent' ? 'مرسلة' : value || '—'}

        size="small"

        sx={{

          backgroundColor: value === 'sent' ? 'var(--color-success-50)' : 'var(--color-primary-50)',

          color: value === 'sent' ? 'var(--color-success-700)' : 'var(--color-primary-700)',

          fontSize: '0.7rem',

          height: 22,

          border: `1px solid ${value === 'sent' ? 'var(--color-success-200)' : 'var(--color-primary-200)'}`,
...

  {
    ...
    format: (value, row) => (
      ...
      <MuiChip
        ...
        label={value ? 'مستخدمة' : 'غير مستخدمة'}

        size="small"

        sx={{

          backgroundColor: value ? 'var(--color-warning-50)' : 'var(--color-info-50)',

          color: value ? 'var(--color-warning-700)' : 'var(--color-info-700)',

          fontSize: '0.7rem',

          height: 22,

          border: `1px solid ${value ? 'var(--color-warning-200)' : 'var(--color-info-200)'}`,
...

  {
    ...
    format: (value, row) => {
      ...
      if (!value || value.length === 0) return <MuiTypography variant="body2">—</MuiTypography>

      const checkedIn = value.filter(g => g.checkedIn).length

      return (

        <MuiBox sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center' }}>

          <UserCheck size={14} style={{ color: 'var(--color-text-secondary)' }} />

          <MuiTypography variant="body2">{checkedIn} / {value.length}</MuiTypography>
                ...
                {invitation.qrCodeImage ? (
                  ...
                  >
                    ...
                    <img
                      ...
                      src={invitation.qrCode ? `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(invitation.qrCode)}&charset-source=UTF-8` : invitation.qrCodeImage}
