...
export default function EventsManagement() {
    ...
    const handleCreateSubmit = async (data) => {
        ...
        if (result.success) {
            ...
            // Invalidate clients query to refresh client list

            queryClient.invalidateQueries({ queryKey: ['manager', 'clients'] })

            // Invalidate events query to refresh events list
            ...
            if (result.error?.response?.data?.errors) {
                ...
                const errorMessages = errors.map(err => err.msg || err.message || err.error || 'خطأ غير معروف')
                ...
                showError('حدث خطأ أثناء إنشاء الفعالية')
    ...
    const handleUpdateSubmit = async (data) => {
        ...
        if (!result.success) {
            ...
            if (result.error?.response?.data?.errors) {
                ...
                showError('حدث خطأ أثناء تحديث الفعالية')
